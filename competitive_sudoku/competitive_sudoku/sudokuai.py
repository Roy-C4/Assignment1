...
class SudokuAI(object):
    ...
    def compute_best_move(self, game_state: GameState) -> None:
        ...
        def minimax(position, depth, max_player, curr_depth):
            if depth == 0 or game_state == 0 or curr_depth == depth:
                return 1 if max_player else -1
            
            if max_player:
                return max(minimax(position*2, depth, False, curr_depth+1), 
                           minimax(((position*2)+1), depth, False, curr_depth+1))
                           
            else:
                return min(minimax(position*2, depth, True, curr_depth+1), 
                           minimax((((position*2)+1), depth, True, curr_depth+1))          
